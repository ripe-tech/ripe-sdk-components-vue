...
| fontTrim              | `Number`   | `false`  | Overrides the profiles's font trim, which defines if the initials are trimmed. |
...
| lineHeight            | `Number`   | `false`  |Overrides the profiles's line height, which defines the initials line height.|
...
| offsets               | `Object`   | `false`  | Overrides the profiles's offset to be applied on the initials. Example: { 0: [0, 6], 1: [0, -10], 2: [0, 10] }.|
